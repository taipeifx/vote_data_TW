...
######################### LOAD 10 ##############################################
elbase_10 = read_csv("elbase.csv",col_names = F) 

elcand_10 = read_csv("elcand.csv",col_names = F)
...
a = elbase_10[elbase_10$X5 == "0000",]
View(a)

write_csv(a[6],"AName10.csv")
...
tks_cand_10 <- read_csv("tks_cand_10.csv")
View(tks_cand_10)
...
tks_cand_10u = unite(data = tks_cand_10,col = CanNames.PParty, c(8:9), sep = ": ", remove = T) #unite CanNames.PParty

#weather3 <- weather2 %>% spread(element, value) #NEEDS POLLN
tks_cand_10us = spread(data = tks_cand_10u, key = CanNames.PParty, value = VotesGot)#spread CanNames.PParty and VotesGot
write_csv(x = tks_cand_10us, path = "tks_cand_10us.csv",col_names = T) #write to mysql uploads for use
View(tks_cand_10us)
